...
export default function OrderActions({
  ...
  const handleAction = async ({ actionName, apiCall, successMessage, destructive = false }) => {
    Alert.alert(actionName, `${actionName} Order №${order.id}?`, [
  ...
  const renderCompactButton = (title, color, onPress) => (
    <TouchableOpacity onPress={onPress} disabled={loadingAction} style={{ marginHorizontal: 4, paddingVertical: 4 }}>
      ...
      <View style={{ flexDirection: 'row', marginTop: 6, alignItems: 'center' }}>
        {isWorker && (
          <>
            {order.isTaken ? (
              <>
                {renderCompactButton('Cancel', '#c32323', () =>
                  handleAction({
                    actionName: 'Cancel',
                ...
                {renderCompactButton('Complete', '#3890ef', () =>
                  handleAction({
                    actionName: 'Complete',
                ...
                {renderCompactButton('Release', 'gray', () =>
                  handleAction({
                    actionName: 'Release',
              ...
              renderCompactButton('Take', 'green', () =>
                handleAction({
                  actionName: 'Take',
        ...
        {isAdmin && !disableAdminActions && (
          <>
            {renderCompactButton('Delete', 'red', () =>
              handleAction({
                actionName: 'Delete',
    ...
    <View style={styles.detailContainer}>
      {isWorker && (
        <View style={styles.buttonOrderDetailsUser}>
          {order.isTaken ? (
            <>
              {renderFullButton('Cancel', '#c32323', () =>
                handleAction({
                  actionName: 'Cancel',
                  apiCall: () => cancelOrder(order.id),
                  successMessage: `Order №${order.id} cancelled`,
              ...
              {renderFullButton('Complete', '#3890ef', () =>
                handleAction({
                  actionName: 'Complete',
                  apiCall: () => completeOrder(order.id),
                  successMessage: `Order №${order.id} completed`,
